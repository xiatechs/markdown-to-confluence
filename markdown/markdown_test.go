...
<p><span class="confluence-embedded-file-wrapped"><img src="` + link + `"></img></span></p>`),
	...
	for _, test := range testInputs {
		...
		t.Run(test.Name, func(t *testing.T) {
			result, _ := markdown.ParseMarkdown(0, test.input, false, 0)
...
<p>test description</p>`),
	...
	out, err := markdown.ParseMarkdown(0, testContent, false, 0)
	assert.Nil(t, err)
...
func TestParseMarkdown_MalformedFrontMatter(t *testing.T) {
	...
	_, err := markdown.ParseMarkdown(0, testContent, false, 0)
	assert.NotNil(t, err)
}
